...
export default function App(){
  ...
  const downloadMission=()=>{
    ...
    const labels:Record<string,string>={cognome:"Cognome",nome:"Nome",dataNascita:"Data di nascita",sesso:"Sesso",anamnesi:"Anamnesi / relazione di soccorso",destinazioneTipo:"Destinazione",destinazioneAltro:"Destinazione specificata",ospedale:"Ospedale"};
    const pretty=(k:string)=>labels[k]||k.replace(/^prest_/,'').replace(/^pres_/,'').replace(/^les_/,'').replace(/_zona$/,' zona').replace(/_intensita$/,' intensità').replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());
    const val=(v:any)=>v===true?'Sì':v===false?'No':Array.isArray(v)?v.join(', '):String(v??'').trim();
    const rows:string[]=[]; const add=(k:string,v:any)=>{const text=val(v);if(text)rows.push(`<tr><th>${escapeHtml(pretty(k))}</th><td>${escapeHtml(text)}</td></tr>`)};
    ["cognome","nome","dataNascita","sesso"].forEach(k=>add(k,s[k])); add("Età",age);
    const eventKeys=["casa","strada","uffici_esercizi","impianto_sportivo","impianto_lavorativo","avvelenamento","evento_violento","precipitato","pedone_ciclo","conducente_moto","malore","travaglio_parto","infortunio","auto","passeggero"];
    rows.push(`<tr><th colspan="2">Evento</th></tr>`); eventKeys.forEach(k=>{if(s[k])add(k,"Selezionato")});
    [0,1,2].forEach(i=>{const e=s[`eval${i}`];if(e&&Object.keys(e).length){rows.push(`<tr><th colspan="2">${i+1}ª valutazione${e.ora?` · ${escapeHtml(e.ora)}`:""}</th></tr>`);["fr","fc","satAria","satO2","pa","temp","glicemia"].forEach(k=>add(k,e[k]));}});
    rows.push(`<tr><th colspan="2">CPSS</th></tr>`); ["deviazione_rima_labiale","segni_di_lato","alterazioni_linguaggio"].forEach(k=>{if(s[k])add(k,"Positivo")});
    rows.push(`<tr><th colspan="2">Prestazioni / interventi</th></tr>`); Object.keys(s).filter(k=>k.startsWith("prest_")&&s[k]).forEach(k=>add(k,"Eseguito"));
    rows.push(`<tr><th colspan="2">Presidi utilizzati</th></tr>`); Object.keys(s).filter(k=>k.startsWith("pres_")&&s[k]).forEach(k=>add(k,"Utilizzato"));
    rows.push(`<tr><th colspan="2">Lesioni</th></tr>`); Object.keys(s).filter(k=>k.startsWith("les_")&&!k.endsWith("_zona")&&!k.endsWith("_intensita")&&s[k]).forEach(k=>{add(k,"Selezionata");if(s[`${k}_zona`])add(`${k}_zona`,s[`${k}_zona`]);if(k==="les_dolore"&&s.les_dolore_intensita)add("les_dolore_intensita",`${s.les_dolore_intensita}/10`)});
    if(s.destinazioneTipo)add("destinazioneTipo",s.destinazioneTipo==="ps"?"Pronto Soccorso":s.destinazioneTipo==="ospedale"?"Ospedale":"Altro"); if(s.ospedale)add("ospedale",s.ospedale); if(s.destinazioneAltro)add("destinazioneAltro",s.destinazioneAltro); if(s.anamnesi)add("anamnesi",s.anamnesi);
    const html=`<!doctype html><html lang="it"><head><meta charset="utf-8"><title>Scheda ${escapeHtml(m.missione_numero)}</title><style>body{font-family:Arial;margin:30px;color:#111}h1{margin-bottom:4px}h2{border-bottom:1px solid #ccc;padding-bottom:5px}table{border-collapse:collapse;width:100%;margin:12px 0}td,th{border:1px solid #ccc;padding:7px;text-align:left}th{width:32%;background:#f5f5f5}.muted{color:#666}</style></head><body><h1>Scheda missione ${escapeHtml(m.missione_numero)}</h1><p><b>${escapeHtml(m.indirizzo)}</b> · ${escapeHtml(m.tipologia||"Missione")} · priorità ${escapeHtml(m.priorita)}</p><p class="muted">${escapeHtml(m.note||"")}</p><h2>Mezzi</h2><table><tr><th>Mezzo</th><th>Targa</th><th>Stato</th></tr>${(m.mezzi||[]).map((z:any)=>`<tr><td>${escapeHtml(z.nome)}</td><td>${escapeHtml(z.targa||"")}</td><td>${escapeHtml(STATUS_LABEL[z.stato]||z.stato)}</td></tr>`).join("")}</table><h2>Scheda compilata</h2><table>${rows.join("")}</table><h2>Cronologia</h2><table><tr><th>Ora</th><th>Mezzo</th><th>Stato</th></tr>${(m.cronologia||[]).map(c=>`<tr><td>${new Date(c.registrato_il).toLocaleString("it-IT")}</td><td>${escapeHtml((m.mezzi||[]).find((z:any)=>z.id===c.mezzo_id)?.nome||"Sistema")}</td><td>${escapeHtml(LABELS[c.stato]||c.stato)}</td></tr>`).join("")}</table></body></html>`;
  ...
  return <div className="app">
    ...
    {selectedMissione&&<div className="drawerBack" onMouseDown={e=>{if(e.target===e.currentTarget)setSelectedMissione(null)}}>
      <section className="drawer">
        <div className="drawerHead"><div><span className={`prio ${selectedMissione.priorita}`}>{selectedMissione.priorita.toUpperCase()}</span><b>{selectedMissione.missione_numero}</b><h2>{selectedMissione.indirizzo}</h2><small>{selectedMissione.tipologia||"Missione"} · {STATUS_LABEL[selectedMissione.stato]||selectedMissione.stato}</small></div><div className="drawerActions"><button onClick={downloadMission}>⇩ Scheda</button>{!["concluso","annullato"].includes(selectedMissione.stato)&&<button className="closeMission" onClick={()=>closeMission(selectedMissione.id)}>✓ Chiudi missione</button>}<button onClick={()=>setSelectedMissione(null)}>×</button></div></div>
        <div className="drawerBody">
          ...
          <section className="detailCard"><h3>Scheda missione</h3>
            ...
            <h4>Lesioni</h4><div className="lesionList">{["amputazione","frattura_esposta","deformita","dolore","sanguinamento","emorragia_massiva","ferita","ferita_penetrante","lacerazione_schiacciamento","contusione","ustione","proiettato","edema","lesioni_incompatibili_vita","accesso_difficile","presenza_deceduti","incastrato","estricazione_20_min","motilita_assente","sensibilita_assente"].map(k=><div className="lesionRow" key={k}><Toggle label={k.replaceAll("_"," ")} checked={!!sheet[`les_${k}`]} onChange={()=>updateSheet(`les_${k}`,!sheet[`les_${k}`])}/>{sheet[`les_${k}`]&&<><input className="lesionZone" placeholder="Zona interessata" value={sheet[`les_${k}_zona`]||""} onChange={e=>updateSheet(`les_${k}_zona`,e.target.value)}/>{k==="dolore"&&<select className="painSelect" value={sheet.les_dolore_intensita||""} onChange={e=>updateSheet("les_dolore_intensita",e.target.value)}><option value="">Intensità</option>{Array.from({length:10},(_,i)=>i+1).map(n=><option key={n} value={n}>{n}/10</option>)}</select>}</>}</div>)}</div>
            <h4>Destinazione / note</h4><div className="destinationRow"><label>Destinazione<select value={sheet.destinazioneTipo||""} onChange={e=>updateSheet("destinazioneTipo",e.target.value)}><option value="">—</option><option value="ps">Pronto Soccorso</option><option value="ospedale">Ospedale</option><option value="altro">Altro</option></select></label>{sheet.destinazioneTipo==="ospedale"&&<label>Ospedale<input value={sheet.ospedale||selectedMissione.ospedale||""} onChange={e=>{updateSheet("ospedale",e.target.value);setSelectedMissione({...selectedMissione,ospedale:e.target.value})}} placeholder="Nome ospedale"/></label>}{sheet.destinazioneTipo==="altro"&&<label>Specificare<input value={sheet.destinazioneAltro||""} onChange={e=>updateSheet("destinazioneAltro",e.target.value)} placeholder="Inserisci destinazione"/></label>}</div><textarea className="wide" value={sheet.anamnesi||""} onChange={e=>updateSheet("anamnesi",e.target.value)} placeholder="Anamnesi / relazione di soccorso"/><button className="save full" onClick={saveSheet}>Salva scheda</button>
          </section>
