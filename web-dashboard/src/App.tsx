...
export default function App(){
  ...
  const [modal,setModal]=useState<"nuova"|"mezzo"|"poi"|"brogliaccio"|null>(null);
  ...
  const updateMissionTimesDirect=async(id:string,field:string,value:string)=>{try{await save(`/interventi/${id}/orari`,"PATCH",{[field]:toIso(value)});await refresh()}catch(e:any){setError(e.message)}};
  const updateEventTimeDirect=async(missioneId:string,eventId:string,value:string)=>{if(!value)return;try{await save(`/interventi/${missioneId}/cronologia/${eventId}`,"PATCH",{registrato_il:toIso(value)});await refresh();if(selectedMissione?.id===missioneId)await openMission(missioneId)}catch(e:any){setError(e.message)}};
  const deleteEventDirect=async(missioneId:string,eventId:string)=>{if(!confirm("Eliminare questo evento dal brogliaccio? L'azione non è reversibile."))return;try{await save(`/interventi/${missioneId}/cronologia/${eventId}`,"DELETE",{});await refresh();if(selectedMissione?.id===missioneId)await openMission(missioneId)}catch(e:any){setError(e.message)}};
  ...
  return <div className="app">
    ...
    <button className="brogliaccioFab" onClick={()=>setModal("brogliaccio")} title="Apri brogliaccio completo">☷ <span>Brogliaccio completo</span></button>
    ...
    {modal&&<div className={`modalBack ${modal==="brogliaccio"?"brogliaccioBack":""}`} onMouseDown={e=>{if(e.target===e.currentTarget)setModal(null)}}><div className={`modal ${modal==="brogliaccio"?"brogliaccioModal":""}`}><button className="close" onClick={()=>setModal(null)}>×</button>{modal==="brogliaccio"?<><div className="brogliaccioHeader"><div><h2>Brogliaccio completo</h2><small>Tutte le missioni della giornata · modifica orari ed elimina eventi direttamente da qui.</small></div><button onClick={downloadBrogliaccio}>⇩ CSV</button></div><div className="brogliaccioList">{missioni.slice().sort((a,b)=>new Date(b.creato_il).getTime()-new Date(a.creato_il).getTime()).map(m=><section className="brogliaccioMission" key={m.id}><div className="brogliaccioMissionHead"><div><b>{m.missione_numero}</b><strong>{m.indirizzo}</strong><small>{m.tipologia||"Missione"} · {STATUS_LABEL[m.stato]||m.stato} · {(m.mezzi||[]).map(z=>z.nome).join(", ")||"Nessun mezzo"}</small></div><button onClick={()=>{setModal(null);openMission(m.id)}}>Apri scheda</button></div><div className="brogliaccioTimes"><label>Creazione<input type="datetime-local" defaultValue={toLocalInput(m.creato_il)} onBlur={e=>updateMissionTimesDirect(m.id,"creato_il",e.target.value)}/></label><label>Assegnazione<input type="datetime-local" defaultValue={toLocalInput(m.ora_assegnazione)} onBlur={e=>updateMissionTimesDirect(m.id,"ora_assegnazione",e.target.value)}/></label><label>Presa in carico<input type="datetime-local" defaultValue={toLocalInput(m.ora_presa_in_carico)} onBlur={e=>updateMissionTimesDirect(m.id,"ora_presa_in_carico",e.target.value)}/></label><label>Arrivo<input type="datetime-local" defaultValue={toLocalInput(m.ora_arrivo)} onBlur={e=>updateMissionTimesDirect(m.id,"ora_arrivo",e.target.value)}/></label><label>Rientro<input type="datetime-local" defaultValue={toLocalInput(m.ora_rientro)} onBlur={e=>updateMissionTimesDirect(m.id,"ora_rientro",e.target.value)}/></label></div>{(m.cronologia||[]).length?<div className="brogliaccioEvents">{m.cronologia!.slice().sort((a,b)=>new Date(a.registrato_il).getTime()-new Date(b.registrato_il).getTime()).map(c=><div className="brogliaccioEvent" key={c.id}><input type="datetime-local" defaultValue={toLocalInput(c.registrato_il)} onBlur={e=>updateEventTimeDirect(m.id,c.id,e.target.value)}/><b>{LABELS[c.stato]||c.stato}</b><span>{(m.mezzi||[]).find(z=>z.id===c.mezzo_id)?.nome||"Sistema"}</span><button className="deleteEvent" onClick={()=>deleteEventDirect(m.id,c.id)}>Elimina</button></div>)}</div>:<div className="empty">Nessun evento di stato registrato.</div>}</section>)}{!missioni.length&&<div className="empty">Nessuna missione per questa giornata.</div>}</div></>:modal==="nuova"?<><h2>Nuova missione</h2><form onSubmit={createMission}><input placeholder="Luogo / indirizzo *" required value={form.indirizzo} onChange={e=>setForm({...form,indirizzo:e.target.value})}/><div className="grid"><select value={form.priorita} onChange={e=>setForm({...form,priorita:e.target.value})}><option value="verde">VERDE</option><option value="giallo">GIALLO</option><option value="rosso">ROSSO</option></select><input placeholder="Tipologia" value={form.tipologia} onChange={e=>setForm({...form,tipologia:e.target.value})}/></div><textarea placeholder="Note" value={form.note} onChange={e=>setForm({...form,note:e.target.value})}/><button className="save full">Crea missione</button></form></>:modal==="mezzo"?<><h2>{mezzoForm.id?"Modifica mezzo":"Nuovo mezzo"}</h2><form onSubmit={createOrEditMezzo}><input placeholder="Nome mezzo" required value={mezzoForm.nome} onChange={e=>setMezzoForm({...mezzoForm,nome:e.target.value})}/><input placeholder="Targa" value={mezzoForm.targa} onChange={e=>setMezzoForm({...mezzoForm,targa:e.target.value})}/><button className="save full">Salva</button></form>{mezzoForm.id&&<button className="danger full" onClick={()=>{deleteMezzo(mezzoForm.id);setModal(null)}}>Elimina mezzo</button>}</>:<><h2>Nuovo punto di interesse</h2><form onSubmit={savePoi}><select value={poiForm.tipo} onChange={e=>setPoiForm({...poiForm,tipo:e.target.value})}>{Object.keys(POI_LABELS).map(k=><option key={k} value={k}>{POI_LABELS[k]}</option>)}</select><input placeholder="Etichetta (es. Squadra Alfa) *" required value={poiForm.etichetta} onChange={e=>setPoiForm({...poiForm,etichetta:e.target.value})}/><input placeholder="Note (opzionale)" value={poiForm.note} onChange={e=>setPoiForm({...poiForm,note:e.target.value})}/><button className="save full">Posiziona sulla mappa</button></form></>}</div></div>}
  </div>;
}
