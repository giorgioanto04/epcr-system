...
type Missione = { id:string; missione_numero:string; indirizzo:string; tipologia?:string; note?:string; priorita:string; ospedale?:string|null; stato:string; creato_il:string; lat?:number|null; lon?:number|null; scheda:any; pazienti?:Paziente[]; mezzi:any[]; cronologia:Evento[] };
...
export default function App() {
  ...
  const loadMission=useCallback(async()=>{
    ...
    try {
      ...
      setMissione(m); const p=(m?.pazienti||[]).find((x:Paziente)=>x.mezzo_id===MEZZO_ID)||(m?.pazienti||[])[0]||null; setPaziente(p); setSheet(p?.scheda||m?.scheda||{});
  ...
  const saveSheet=async()=>{if(!missione||!paziente)return;try{const updated=await api(`/interventi/${missione.id}/pazienti/${paziente.id}`,"PATCH",{scheda:sheet,mezzoId:paziente.mezzo_id||MEZZO_ID});setPaziente(updated);await loadMission();Alert.alert("Scheda",`Scheda paziente ${updated.numero} salvata`);}catch(e:any){Alert.alert("Scheda",e.message)}};
  const addPatient=async()=>{if(!missione)return;try{const p=await api(`/interventi/${missione.id}/pazienti`,"POST",{mezzoId:MEZZO_ID,etichetta:`Paziente ${(missione.pazienti||[]).length+1}`});await loadMission();setPaziente(p);setSheet(p.scheda||{});}catch(e:any){Alert.alert("Paziente",e.message)}};
  const selectPatient=(p:Paziente)=>{setPaziente(p);setSheet(p.scheda||{})};
  const deletePatient=async()=>{if(!missione||!paziente)return;Alert.alert("Elimina paziente",`Eliminare definitivamente la scheda Paziente ${paziente.numero}? La missione rimane intatta.`,[{text:"Annulla",style:"cancel"},{text:"Elimina",style:"destructive",onPress:async()=>{try{await api(`/interventi/${missione.id}/pazienti/${paziente.id}`,"DELETE");await loadMission();}catch(e:any){Alert.alert("Paziente",e.message)}}] )};
  ...
  return <View style={styles.container}>
    ...
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      <View style={styles.card}><View style={styles.patientHead}><View><Text style={styles.sectionTitle}>Pazienti della missione</Text><Text style={styles.muted}>Ogni paziente ha una scheda indipendente</Text></View><Pressable style={styles.patientAdd} onPress={addPatient}><Text style={styles.patientAddText}>＋ Nuovo</Text></Pressable></View><View style={styles.stateWrap}>{(missione.pazienti||[]).map(p=><Pressable key={p.id} onPress={()=>selectPatient(p)} style={[styles.stateBtn,paziente?.id===p.id&&styles.stateBtnOn]}><Text style={paziente?.id===p.id?styles.stateTextOn:undefined}>Paziente {p.numero}</Text></Pressable>)}</View><Text style={styles.muted}>{paziente?`Scheda paziente ${paziente.numero} · ${missione.mezzi.find((m:any)=>m.id===paziente.mezzo_id)?.nome||"equipaggio"}`:"Seleziona una scheda"}</Text>{paziente&&<Pressable style={styles.deletePatient} onPress={deletePatient}><Text style={styles.deletePatientText}>Elimina scheda paziente</Text></Pressable>}</View>
...
const styles=StyleSheet.create({container:{flex:1,backgroundColor:"#f3f6fa"},header:{paddingTop:48,paddingHorizontal:18,paddingBottom:12,backgroundColor:"#0f172a",flexDirection:"row",justifyContent:"space-between",alignItems:"center"},brand:{fontSize:21,fontWeight:"500",color:"#fff"},muted:{color:"#94a3b8",marginTop:3},refresh:{backgroundColor:"#fff",borderRadius:10,paddingHorizontal:13,paddingVertical:8},scroll:{padding:14,paddingBottom:40},center:{flex:1,justifyContent:"center",alignItems:"center",padding:30},title:{fontSize:22,fontWeight:"500",textAlign:"center"},card:{backgroundColor:"#fff",borderRadius:14,padding:15,marginBottom:12,borderWidth:1,borderColor:"#e2e8f0"},missionNumber:{fontSize:13,fontWeight:"500",color:"#2563eb"},address:{fontSize:23,fontWeight:"500",marginTop:4},meta:{color:"#64748b",marginTop:4},alert:{backgroundColor:"#fee2e2",padding:16,borderBottomWidth:1,borderBottomColor:"#fecaca"},alertTitle:{fontSize:26,fontWeight:"500",color:"#b91c1c"},alertMission:{fontWeight:"500",marginTop:3},alertAddress:{fontSize:19,fontWeight:"500",marginVertical:8},patientAddText:{color:"#fff",fontWeight:"500"},deletePatient:{marginTop:9,borderWidth:1,borderColor:"#fecaca",backgroundColor:"#fff1f2",padding:9,borderRadius:9,alignItems:"center"},deletePatientText:{color:"#b91c1c",fontWeight:"500"},patientHead:{flexDirection:"row",justifyContent:"space-between",alignItems:"center",marginBottom:10},patientAdd:{backgroundColor:"#2563eb",paddingHorizontal:10,paddingVertical:8,borderRadius:9},sectionTitle:{fontSize:18,fontWeight:"500",marginBottom:10},subTitle:{fontSize:15,fontWeight:"500",marginTop:16,marginBottom:8},current:{marginBottom:10,color:"#475569"},bold:{fontWeight:"500",color:"#0f172a"},stateWrap:{flexDirection:"row",flexWrap:"wrap",gap:7},stateBtn:{borderWidth:1,borderColor:"#cbd5e1",borderRadius:9,paddingVertical:9,paddingHorizontal:10,backgroundColor:"#fff",marginBottom:6},stateBtnOn:{backgroundColor:"#2563eb",borderColor:"#2563eb"},stateTextOn:{color:"#fff",fontWeight:"500"},timeline:{borderLeftWidth:3,borderLeftColor:"#2563eb",paddingLeft:10,paddingVertical:6,marginBottom:6},time:{fontSize:12,color:"#64748b"},event:{fontWeight:"500",marginTop:2},field:{marginBottom:9},label:{fontSize:12,color:"#475569",marginBottom:4},input:{borderWidth:1,borderColor:"#cbd5e1",borderRadius:9,paddingHorizontal:10,paddingVertical:9,backgroundColor:"#fff"},multiline:{minHeight:110,textAlignVertical:"top"},checkWrap:{flexDirection:"row",flexWrap:"wrap",gap:6},check:{borderWidth:1,borderColor:"#cbd5e1",borderRadius:8,padding:8,backgroundColor:"#fff"},checkOn:{backgroundColor:"#dbeafe",borderColor:"#60a5fa"},lesionRow:{marginBottom:6,borderBottomWidth:1,borderBottomColor:"#eef2f7",paddingBottom:6},save:{marginTop:15,backgroundColor:"#2563eb",padding:13,borderRadius:10,alignItems:"center"},saveText:{color:"#fff",fontWeight:"500",fontSize:16}});
